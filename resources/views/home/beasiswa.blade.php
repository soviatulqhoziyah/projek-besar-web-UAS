...
<div class="main-container">
    @if($beasiswas->count())
            ...
            @foreach($beasiswas as $beasiswa)
                <div class="event-card">
                    <div class="event-image">
                        @php $img = $beasiswa->poster; @endphp
                        @if(Str::startsWith($img, ['http://', 'https://']))
                            <img src="{{ $img }}" alt="{{ $beasiswa->judul }}">
                        @else
                            <img src="{{ asset('storage/'.$img) }}" alt="{{ $beasiswa->judul }}">
                        @endif
                        <div class="event-badge"><i class="fas fa-graduation-cap me-1"></i>Beasiswa</div>
                    </div>

                    <div class="event-content">
                        <h3 class="event-title">{{ $beasiswa->judul }}</h3>
                        <p class="event-description">{{ Str::limit($beasiswa->deskripsi, 100) }}</p>

                        <div class="event-details">
                            <div class="detail-item">
                                <div class="detail-icon"><i class="fas fa-calendar"></i></div>
                                <span>Mulai: {{ \Carbon\Carbon::parse($beasiswa->tanggal_mulai)->translatedFormat('d F Y') }}</span>
                            </div>
                            <div class="detail-item">
                                <div class="detail-icon"><i class="fas fa-calendar-check"></i></div>
                                <span>Berakhir: {{ \Carbon\Carbon::parse($beasiswa->tanggal_berakhir)->translatedFormat('d F Y') }}</span>
                            </div>
                        </div>

                        <div class="event-actions">
                            @if($beasiswa->link_daftar)
                            <a href="{{ $beasiswa->link_daftar }}" target="_blank" class="btn-modern btn-primary">
                                <i class="fas fa-user-plus me-2"></i>Daftar Sekarang
                            </a>
                            @endif
                            <a href="{{ route('beasiswa.show', $beasiswa->id) }}" class="btn-modern btn-outline">
        ...
        <div class="pagination-wrapper">{{ $beasiswas->links() }}</div>
    @else
        <div class="empty-state">
            <i class="fas fa-calendar-times"></i>
            <h3>Belum Ada Info Beasiswa</h3>
            <p>Maaf, belum ada informasi beasiswa yang tersedia saat ini.</p>
        </div>
    @endif
</div>
